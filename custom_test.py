...
# tensor_image = tensor_image.repeat(2, 1, 1, 1)
# image = tensor_image.unsqueeze(2).repeat(1, 1, 2, 1, 1)
...
def test_pi_zero_with_vit():
    ...
    # torch.randint(0, 20_000, (1, 1024)) arr2tensor(state)
    commands = text2tokens(text, vocab)
    joint_state = arr2tensor(state)  # torch.randn(1, 6)
    actions = arr2tensor(action).unsqueeze(0)  # torch.randn(1, 1, 6)
